...
export default function GitDropdown({ repoState }: DropdownProps) {
        ...
        >
          <div className="flex items-center justify-between space-x-4 px-4">
            <div className="flex items-center justify-start space-x-4 p-0">
              <Folder />
              <span
                className={`text-md ${file.staged ? "" : "text-yellow-300"}`}
              >
                {file.filename}
              </span>
            </div>
  ...
  return <>{filetree?.files.map((file) => traverseFile(file))}</>;
}
